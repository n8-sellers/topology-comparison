...
const DeviceFormDialog = ({ 
  ...
  const [portConfigDialogOpen, setPortConfigDialogOpen] = useState(false);
  const [currentPortConfig, setCurrentPortConfig] = useState(null);
  const [currentPortConfigIndex, setCurrentPortConfigIndex] = useState(-1);
  const [breakoutDialogOpen, setBreakoutDialogOpen] = useState(false);
  const [currentBreakout, setCurrentBreakout] = useState('');
  const [downlinkDialogOpen, setDownlinkDialogOpen] = useState(false);
  const [currentDownlink, setCurrentDownlink] = useState('');
  ...
  useEffect(() => {
    if (open) {
      if (device) {
        ...
        const template = createDeviceTemplate(deviceType);
  ...
  const validateForm = () => {
    ...
    if (deviceType === 'leaf' && (!formData.downlinkOptions || formData.downlinkOptions.length === 0)) {
  ...
  const openPortConfigDialog = (config = null, index = -1) => {
  ...
  const openBreakoutDialog = () => {
    setCurrentBreakout('');
    setBreakoutDialogOpen(true);
  };

  const closeBreakoutDialog = () => {
    setCurrentBreakout('');
    setBreakoutDialogOpen(false);
  };

  const addBreakoutOption = () => {
  ...
  const removeBreakoutOption = (option) => {
  ...
  const openDownlinkDialog = () => {
    setCurrentDownlink('');
    setDownlinkDialogOpen(true);
  };

  const closeDownlinkDialog = () => {
    setCurrentDownlink('');
    setDownlinkDialogOpen(false);
  };

  const addDownlinkOption = () => {
    if (currentDownlink) {
      const updatedDownlinks = [...(formData.downlinkOptions || [])];
  ...
  const removeDownlinkOption = (option) => {
    const updatedDownlinks = formData.downlinkOptions.filter(o => o !== option);
          ...
          <Grid container spacing={3} sx={{ mt: 1 }}>
            ...
            {deviceType === 'leaf' && (
              <Grid item xs={12}>
                ...
                <Box sx={{ mt: 2 }}>
                  {formData.downlinkOptions?.map((option) => (
                    <Chip 
                      key={option} 
                      label={option} 
                      sx={{ mr: 1, mb: 1 }} 
                  ...
                  {(!formData.downlinkOptions || formData.downlinkOptions.length === 0) && (
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            <Grid item xs={12} md={6}>
              <TextField
                ...
                onChange={(e) => setCurrentPortConfig({
                  ...currentPortConfig,
        ...
        <DialogContent>
          <FormControl fullWidth sx={{ mt: 1 }}>
            ...
            <Select
              ...
              onChange={(e) => setCurrentBreakout(e.target.value)}
            >
        ...
        <DialogContent>
          <FormControl fullWidth sx={{ mt: 1 }}>
            ...
            <Select
              ...
              onChange={(e) => setCurrentDownlink(e.target.value)}
            >
              <MenuItem value="10G">10G</MenuItem>
              <MenuItem value="25G">25G</MenuItem>
              <MenuItem value="40G">40G</MenuItem>
